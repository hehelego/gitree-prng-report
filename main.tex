...
\usepackage{stmaryrd}
\usepackage{xcolor}
\definecolor{sblue}{cmyk}{1,.72,0,.38}
\definecolor{lightTeal}{cmyk}{.2,0,.1,0}
\definecolor{lightGrey}{gray}{.85}
\definecolor{lightMagenta}{cmyk}{0,.5,0,.1}
...
\newcommand{\prngCtx}{\knowInv{\iota_\prng}{\exists \sigma.\; \hasstate(\sigma) * \hasprngs(\sigma)}}
\newcommand{\heapCtx}{\knowInv{\iota_h}{\exists \sigma_h.\; \mathrm{has\_heap}(\sigma_h)}}
\newcommand{\LitV}{\mathrm{LitV}}
\newcommand{\LitPrng}{\mathrm{LitPrng}}
\newcommand{\safety}{\mathrm{s}}
...
\newcommand{\natopinterp}[3]{\llbracket #1 \rrbracket(#2,#3)}
...
\begin{enumerate}
  ...
  \item Subsequently, I studied the research papers \cite{esop25git-callcc,popl24gitree} to learn the general theory of GItrees. I learned using GItrees to interpret programs involving higher-order effects.
...
\begin{mathpar}
  ...
  \and \inferrule[\(\later\)-mono]{P\vdash Q}{\later P \vdash \later Q}
...
The familiar was Birkedal et al.~\cite{birkedal2012first} proposed the synthetic step-indexing technique in its current form one can found in Iris.
...
The topos of tree gives rise to an intuitionistic higher-order logic with the later modality and a dependent type theory with a later operator.
...
Moreover, reasoning and manipulation or resources in Iris are localized.
...
As demonstrated in by Jung et al.~\cite{iris3}, many fundamental concepts in concurrent separation logic do not have to be introduced as primitives,
instead they can be defined as ghost ownership of carefully crafted resource algebras.
Instance of which includes the points-to predicate, and (surprisingly) invariants.
\begin{mathpar}
...
The flexible method can be used to establish soundness of a type system i.e. execution of well-typed programs never get stuck or result in an errorneous state or investigate specific programs, for example, proving a program safely encapsulate unsafe operations or proving contextual equivalence of two programs.
...
Despite being a scalable and powerful method, SKLR has some drawbacks. Proofs are often obscured and hard to mechanized due to the involvement of global state manipulations on the Kripke frame and tedious step-index arithmetics. These complications can be drastically reduced by defining logical relations in a suitable metatheory that can handle step-indexing and frame-preserving state manipulation abstractly.
...
The expression relation is defined as \(\semEtype{\Tenv}{\typ}{\semenv}(\expr) \eqdef \wpre{e}{\semVtype{\Tenv}{\typ}{\semenv}}\), stating that \(e\) is will not get stuck and may terminate as a value in \(\semVtype{\Tenv}{\typ}{\semenv}\).
The value relation, capturing concerns the shape and hereditary semantic property of a value of the type in question, is define as follows:
...
\begin{enumerate}
  ...
  \item Quantifying over reducibility candidates.
    When interpreting \(\forall \tvar.\; \typ\) and \(\exists \tvar.\; \typ\), we did not directly quantify over of all syntactic types, which would introduces circularity.
...
The Stanford Encyclopedia of Philosophy has a page on game semantics~\cite{sep-games-abstraction} which contains a detailed discussing of the connection between denotational semantics and operational semantics
...
Guarded Interaction trees belongs to a long-standing tradition of modeling and reasoning about effectful programs monads to programs within total type theories, an approach pioneered by Moggi~\cite{moggi-monad}.
...
Starting form now, we implicitly work within the logic of Iris. When we say a type, we mean an OFE or a COFE. A function type should be understood as the space of non-expansive maps between two OFEs, which is itself an OFE.
...
For each effect \(i \in E\), \(\Ins_i, \Outs_i : \COFE \to \COFE\) are two functors specifying the expected input and output type of effect \(i\).
...
\begin{itemize}
  ...
  \item A collection of smart constructors for GItrees can be defined using the fold morphism.
    \begin{align*}
      ...
      \gVis &: \forall (op : I) (inputs : Ins_i(\latert \IT)) (callback : Outs_i(\latert \IT) \to \latert \IT) \to \IT
    \end{align*}
  \item We assume that the value type \(A\) is a coproduct. We write \(\gRet(b)\) for \(\gRet(\vinj{}{b})\) whenever \(b:B\) and \(\vinj{}{} : B\hookrightarrow A\).
  ...
  \item A function \(\IT_E(A) \to \IT_E(A)\) is called a homomorphism if it satisfies the following equations:
    ...
    \]
    The identity function is a homomorphism and the composition of homomorphisms is homomorphism.
...
\begin{mathpar}
  ...
  \and
  \inferrule[wp-hom]{\text{IT-Hom}(f) \\ \wpre{\alpha}{\alpha_v . \wpre{f(\alpha_v)}{\Phi}}}{\wpre{f(\alpha)}{\Phi}}
  \and
...
These proof rules should be mostly familiar. GItree homomorphisms can be viewed as evaluation contexts. The \textsf{WP-VAL} rule is the monadic return and the \textsf{WP-HOM} rule is the monadic bind.
...
However, it is often desirable to reason about effects with partial knowledge of the state. To illustrate this, consider the familiar \textsf{HEAPLANG-WP-LOAD} rule.

\begin{mathpar}
  \inferrule[HEAPLANG-WP-LOAD]{l\pointsto v}{\wpre{!l}{u . u = v}}
\end{mathpar}

The proof rule states that a partial view of the heap \(l\pointsto v\) is enough to conclude the safety of a load operation. To recover this kind of reasoning principles, one has to design an accompanying ghost theory, which will be exemplified later sections.\par
...
To enable constructing and reasoning about programms that uses multiple effects, a mechanism for composing effect is required.\par
...
Based on the this reifier, we can recover the \textsf{WP-REIFY} program logic proof rule.
...
This is lead to the notion of \emph{sub-effect}.
...
We note that generator values are first-class citizen of the \(\lambda_\prng\) language, which can be freely copied, discarded, passed as arguments or returned from functions.
Indeed, a generator is duplicated when being pass to a function that uses its argument multiple times.
...
\begin{definition}[\(\lambda_{\prng}\) type system]
  ...
  \end{mathpar}
\end{definition}
The type rules is mostly standardly and upholds the weakening and substitution lemma. We note that no typing rule is given for \(\kw{DelPrng}\) -- deleting a potentially shared generator is unsound, so the constructor remains unused in well-typed programs. While it is possible to give a proper typing rule for \(\kw{DelPrng}\) by adopting a linear type discpline, we would like to not overcomplicate \(\lambda_\prng\).

\subsection{Modeling the PRNG effect in guarded interaction trees}

Here, we following the recipe in \cite{esop25git-callcc} to model the PRNG effects.
...
Recall that the reifier \(r_e\) of an effect \(e \in E\) ought to be a function \(Ins_{e}(X) \times State(X) \to \option{( Outs_{e}(X) \times State(X) )} \)

\begin{align*}
  r_{\new}((), \sigma) & \eqdef \Some(l, \sigma[l\mapsto 0]) \text{where \(l \notin \dom(\sigma)\) is a fresh location}\\
...
\begin{itemize}
  ...
  \item \(\mathrm{PRNG\_DEL\_k}(l : \iLoc, k : \mathbf{1} \to X) \eqdef \gVis_{\del}(l, \Next \circ k)\)
    and \(\mathrm{PRNG\_DEL}(l,n) \eqdef \mathrm{PRNG\_DEL\_k}(l, n, \gRet())\)
  \item \(\mathrm{PRNG\_SEED\_k}(l : \iLoc, n : \mathbb{N}, k: \mathbf{1} \to X) \eqdef \gVis_{\new}((l,n), \Next \circ k)\)
...
Equipped with these high-level programming support, one can easily encode randomized algorithms in GItrees.
Consider the following GItree which uses the PRNG effect and the higher-order store effect:
...
This GItree encodes a Las Vegas algorithm: given an initial guess
\(init \in \mathbb{N}\), a seed \(sd \in \mathbb{N}\), and a test
predicate \(test : \mathbb{N} \to \mathbf{2}\) encoded as a GItree,
it creates a generator \(r\), seeds it with \(sd\), allocates a store
cell holding \(init\), and loops: while \(test\) fails on the current
value, it samples a fresh random number from \(r\) and stores it.
When \(test\) succeeds, the loop exits and returns the satisfying value.
The macro \(\mathrm{LET}\;e_1\;e_2\) denotes \(\eapp{(\lambda\cdot\;e_2)}{e_1}\)
and \(\mathrm{SEQ}\;e_1\;e_2\) denotes \(e_1 \cbvap (\lambda\cdot\; e_2)\).
...
To conclude this part, we showcase a proof that the Las Vegas GItree program always finds a valid solution upon termination.  The Coq mechanization of this proof is in \texttt{prng\_examples.v}.

\begin{theorem}[Partial correctness of Las Vegas]
  Let \(f : \mathbb{N} \to \mathbf{2}\) be a predicate, \(test\) a GItree
  that decides \(f\) (i.e., \(\forall n.\; test \cbvap \gRet(n)\) reduces to
  \(\gRet(1)\) if \(f(n)\) holds and \(\gRet(0)\) otherwise), and
  \(sd, init \in \mathbb{N}\).  Then
  \[
  \heapCtx \star \prngCtx \proves
  \wpre{\syn{las\_vegas}(sd, init, test)}
    {\lambda \beta.\; \exists n.\; \beta \equiv \gRet(n) \land f(n)}.
  \]
\end{theorem}
...
\begin{proof}
  ...
  \wpre{\syn{las\_vegas}(sd, init, test)}
    {\lambda \beta.\; \exists n.\; \beta \equiv \gRet(n) \land f(n)}.
  \]
  We reason through the definition step by step.

  \textbf{Step 1: Create and seed the generator.}
  Apply \textsc{WP-PRNG-NEW} to allocate a fresh generator \(r\).
  This gives \(\knownprng(r)\) and updates the PRNG ghost state.
  Then apply \textsc{WP-ALLOC} (the store allocation rule) to allocate
  a heap cell \(s\) containing \(\gRet(init)\), giving the points-to
  predicate \(s \pointsto init\).  By \textsc{WP-SEQ} followed by
  \textsc{WP-PRNG-SEED}, we seed the generator with \(sd\).  The goal
  reduces to proving the while-loop:
  \[
  \wpre{\gwhile\;(\mathrm{neg\_bool}\;(test \cbvap \mathrm{READ}\;s))
                 \;(\mathrm{LET}\;(\mathrm{PRNG\_GEN}\;r)\;(\mathrm{WRITE}\;s))}
        {\lambda \_.\; \wpre{\mathrm{READ}\;s}
          {\lambda \beta.\; \exists n.\; \beta \equiv \gRet(n) \land f(n)}}.
  \]

  \textbf{Step 2: L\"ob induction.}
  Since \(\gwhile\) is defined via guarded recursion, we use L\"ob
  induction.  Assume the induction hypothesis:
  \[
  \IH \equiv \later\wpre{\gwhile\;(\cdots)}{ \lambda \_.\;
    \wpre{\mathrm{READ}\;s}{\lambda \beta.\; \exists n.\;
      \beta \equiv \gRet(n) \land f(n)} }.
  \]
  Unfolding \(\gwhile\):
  \[
  \gwhile(\alpha)(\beta) \equiv
    \gif(\alpha)(\beta \cbvap \Tick(\gwhile(\alpha)(\beta)))(\gRet(())).
  \]
  Here \(\alpha \eqdef \mathrm{neg\_bool}\;(test \cbvap \mathrm{READ}\;s)\)
  and \(\beta \eqdef \mathrm{LET}\;(\mathrm{PRNG\_GEN}\;r)\;
  (\mathrm{WRITE}\;s)\).

  \textbf{Step 3: Read and test.}
  Apply \textsc{WP-READ} to read the current value \(n\) from the store
  cell \(s\), preserving \(s \pointsto n\).  The test predicate
  \(test\) satisfies:
  \[
  \forall n.\; \wpre{test \cbvap \gRet(n)}
    {\lambda \gamma.\; \gamma \equiv \gRet(1) \lor
                      \gamma \equiv \gRet(0)}.
  \]
  Applying the test to \(n\) and using \(\mathrm{neg\_bool}\), the
  condition \(\alpha\) evaluates to either \(\gRet(0)\) (test passed)
  or \(\gRet(1)\) (test failed).  We perform case analysis.

  \textbf{Step 4: Case analysis on the test outcome.}
  \begin{itemize}
    \item \textbf{Test passed} (\(f(n)\) holds): then
      \(\alpha \equiv \gRet(0)\) and the \(\gif\) reduces to
      \(\gRet(())\).  By the sequencing rule we proceed to
      \(\mathrm{READ}\;s\), which returns \(n\).  This satisfies
      the post-condition with witness \(n\) since \(f(n)\) holds.

    \item \textbf{Test failed} (\(f(n)\) does not hold): then
      \(\alpha \equiv \gRet(1)\) and the \(\gif\) reduces to
      \(\beta \cbvap \Tick(\gwhile(\alpha)(\beta))\).
      Applying the CBV application rule, we execute
      \(\mathrm{LET}\;(\mathrm{PRNG\_GEN}\;r)\;(\mathrm{WRITE}\;s)\):
      \textsc{WP-PRNG-GEN} samples a fresh number \(m\) from the
      generator, and \textsc{WP-WRITE} stores \(m\) into the cell \(s\),
      updating the points-to predicate to \(s \pointsto m\).
      A tick step discharges the \(\later\) in the induction hypothesis
      \(\IH\), which we apply with the updated state to complete the
      loop iteration.
  \end{itemize}
  Throughout, the GItree equational theory and homomorphism rules are
  used to expose head-normal forms for symbolic execution.
\end{proof}

It is worth mentioning that the effect signature can be given alternative semantics. More specifically, it can be reified as non-deterministic choice or sampling from a discrete probabilistic distribution. Depending on how the effect is how the effect is interpreted, different program logics rule can be derived, enabling reasoning about GItree programs that involves non-determinism or probabilistic behavior. Unfortunately, these two alternative reifications cannot be implemented in the current formalization since reifiers have to be a Rocq function, which is pure and total.\par
Finally, we mention that the implicit state WP rules is sound for all three possible reifications as it requires the program continue execution safely regardless of the number being generated. Programs that are proven to be safe with this set of rules are safe no matter how we choose to interpret the effect. Specifically, the Las Vegas program will remain to be safe if we replace the PRNG reifier with a probabilistic sampling reifier.

\subsection{Interpreting \(\lambda_\prng\) in guarded interaction trees}

Now that a theory of PRNG effects is in place, we can give a denotational semantics of \(\lambda_\prng\) by interpreting \(\lambda_\prng\) programs as GItrees with PRNG effects. We define a map \(\sem{-}_\senv\) from \(\lambda_\prng\) expresisions to \(\IT_{\prng}(\mathbf{1} + \mathbb{N} + \iLoc)\), where \(\senv:\iVar \to \IT\) is a substitution environment for free variables. The interpretation is depicted in the following figure and should be straight-forward.
\[
  \begin{aligned}
    ...
    \sem{\recval{f}{x}{\expr}}_\senv
                  ...
                  \gFun\left(\later\!\circ\!\left(\lambda \alpha'\;v.\;
...
where \(\fix : \latert \IT \to \IT\) is the guarded fixed point combinator;
\(\later\!\circ\) transforms the function of type \(\IT \to (\IT \to \IT)\) into \(\latert \IT \to \latert (\IT \to \IT)\) so that \(t : \latert \IT\) can be applied;
...
We also interpret evaluation contexts as functions \(\IT\to\IT\).
...
\[
  \begin{aligned}
    ...
    \sem{K \oplus e}_\env(\alpha) &\eqdef \gnatop{\oplus}(\sem{K}_\senv(\alpha))(\sem{e}_\senv)
    \quad \sem{v \oplus K}_\env(\alpha) \eqdef \gnatop{\oplus}(\sem{e}_\senv)(\sem{K}_\senv(\alpha))\\
...
We can show that \(\sem{K[e]} \equiv \sem{K}(\sem{e})\), which means our interpretation is compositional.
Using the GItree combinator equational reasoning rules, we can prove that \(\lambda_\prng\) evaluation contexts corresponds to GItree evaluation contexts, i.e., GItree homomorphisms.\par


Having defined an interpretation of \(\lambda_\prng\) programs in terms of GItrees,
we would like to show that \(\sem{e}\) faithfully and adequately captures the operational semantics by relating the execution of \(e\) and the reduction of \(\sem{e}\),
which will allow us to leverage the interpretation to prove semantical properties, e.g. type safety and contextual refinement, without directly working on the operational semantics.\par
...
\begin{corollary}[Denotational equivalence implies contextual equivalence]
  ...
  for every context \(C\), which is not necessarily an evaluation context, such such that \(\typed{\bullet}{C[e_1]}{\tnat}\) and \(\typed{\bullet}{C[e_2]}{\tnat}\),
  \[
...
\begin{definition}[Binary logical relation for adequacy]
  ...
  \begin{align*}
    ...
    \mathcal{E}_{\type_2}(\gFun(f)\cbvap \beta_v,\; \eapp{v}{v'})\\
...
As hinted at the beginning of this subsection, we can investigate properties of \(\lambda_\prng\) by operating at the level of GItree denotational semantics, circumventing the operational semantics of the language. We showcase this approach with a partial proof of the type safety of \(\lambda_\prng\).\par
...
This is the kind of adequacy theorems discussed in program logics \`a la carte\cite{proglog-carte}.
The research paper proposed a modular approach of building program logic for effectful languagees.
...
\section{Modeling interactions with physical devices}

% TODO: expand ... explain how this is a generalization of the 

\section{Discussion and Future Works}

This project has explored the application of guarded interaction trees as a semantic domain for effectful higher-order languages. Specifically, we have developed a computationally adequte denotational semantics for a functional language equipped with PRNG effects.\par

While the project is overall successful, my solution to several technical difficulties are not completely satisfying and requires future refinement.

\begin{enumerate}
  \item The computational adequacy result is not strong enough.
    From \(\sem{e}\Redu^\ast (\gRet(n),\sigma')\), one can derive \((e,\sigma) \redu^\ast (n,\sigma')\) but gains no knowledge on how \(\sigma\) evolves to \(\sigma'\).
    This will be insufficient when one want to prove semantic properties involving observation of the state during the execution.
    For instance, program indistinguishability cannot be proved if the threat model allows the attack to inspect the store during program execution.
    % TODO: rephrase this.
    % It can be difficult to do so. But we probably imagine it impossible.
    % frame this thing as: some strengthening is needed before we can use this theorem to prove indistinguishability stuff.
    % some full-abstraction results
  \item While the effect interpretations and program logic rules are modularly defined,
    the effectful programming language itself is not constructed in a modular approach.
    As a result, one cannot easily construct a language integrating I/O tape, higher-order store and PRNG by piecing three languages together.
  \item \(\syn{SEED}\) is an operator taking more than one argument.
    It is tricky to find a appropriate GItree interpretation that reduces generate only a single tick.
  \item No program logic rules is derived for \(\mathrm{PRNG\_DEL}\), and no typing rule is defined for \(\syn{DelPrng}\).
    The root cause is that no restriction on sharing of generators is imposed.
    The benefit of this design is that \(\knownprng(\ell)\) becomes persistent so proofs were mostly straight-forward, however, it indeed limits the expressiveness of our program logic.
    One way to lift this limitation is to use fractional ownership, but I haven't experimented with that.
    Once a finer effect theory is developed, one can experiment proving soundness of a qualitative type system for \(\lambda_\prng\), which allows deletion of exclusively owned generators.
  \item An alternative way of building the PRNG effect is to reuse the higher-order store effect.
    I explored this approach initially but abandoned it later for technical difficulties.
  \item I didn't experiment interpreting the effect as non-deterministic or probabilistic generation.
  \item Weaker bisimulation of GItrees as a more helpful equivaleence notion.
    pertaining to the previous point of multiple ticks.
  \item less ideal effect handling ...
    Unify SubOfe, SubEff, and SubReifiers.

\end{enumerate}

Looking forward, the GItree framework provides a promising substrate for reasoning about cyber-physical systems. Future works could investigate the modeling and reasoning of interaction between controllers and physical devices, in the presence of real-time constraints, non-deterministic responses, and probabilistic actions.
As explained before, the theory of GItrees has to be generalized to allow defining reifiers as not merely a function in the metatheory. This will require rephrasing great part of existing works either in written down informally on paper or formalized in Rocq.
There are multiple ways to extend the theory, and it is unclear which one would be optimal:
(1) One can change reifiers to a binary relation over \((\IT,State)\) with richer information. The resulting theory is more operational in the sense reasoning about GItrees will be carried out in an operational style.
(2) One can allow defining reifiers as an monadic function and endow GItrees with trace semantics. This approach likely gives a more denotational theory.

\section{formalization efforts}

\section{Declaration on Generative AI usage}

In the report write-up phase, I used Generative AI to create proof trees from formal proofs mechanized in Rocq.

\section{acknowledgement}

We thank Sergei for discussion and some assistance in writing and proofreading.
